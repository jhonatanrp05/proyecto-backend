  ...
  async execute(
    ...
    if (challenge.createdBy !== professorId) {
      throw new ForbiddenException(
        'Solo el profesor que creó el reto puede cargar el esquema.',
      );
    }
    ...
    if (!tempSchema.isValid()) {
      throw new BadRequestException(
        'El script DDL debe contener al menos un CREATE TABLE.',
      );
    }
...
export class UploadSeedDataUseCase {
  ...
  async execute(
    ...
    if (challenge.createdBy !== professorId) {
      throw new ForbiddenException(
        'Solo el profesor que creó el reto puede cargar datos de prueba.',
      );
    }
    ...
    if (!tempSeed.isValid()) {
      throw new BadRequestException(
        'El script debe contener al menos un INSERT INTO.',
      );
    }
...
export class SetExpectedResultUseCase {
  ...
  async execute(
    ...
    if (challenge.createdBy !== professorId) {
      throw new ForbiddenException(
        'Solo el profesor que creó el reto puede definir el resultado esperado.',
      );
    }

    if (!outputJson || outputJson.length === 0) {
      throw new BadRequestException(
        'El resultado esperado no puede estar vacío.',
      );
    }

    return this.challengeRepo.upsertExpectedResult(
      challengeId,
      query,
      outputJson,
    );
